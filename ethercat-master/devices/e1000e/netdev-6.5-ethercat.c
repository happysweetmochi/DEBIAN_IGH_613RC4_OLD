...
#include "e1000-6.5-ethercat.h"
#define CREATE_TRACE_POINTS
#include "e1000e_trace-6.5-ethercat.h"
			       ...
			       int work_to_do)
{
	...
	staterr = le32_to_cpu(rx_desc->wb.upper.status_error);
	buffer_info = &rx_ring->buffer_info[i];
	//printk("e1000_clean_rx_irq staterr = %d" , staterr);
...
static bool e1000_clean_tx_irq(struct e1000_ring *tx_ring)
{
	...
	eop_desc = E1000_TX_DESC(*tx_ring, eop);
 	//printk("eop_desc->upper.data = %d , cpu_to_le32(E1000_TXD_STAT_DD) = %d , count = %d , tx_ring->count = %d" ,
 	//		eop_desc->upper.data,cpu_to_le32(E1000_TXD_STAT_DD),count,tx_ring->count);
				  ...
				  int work_to_do)
{
	...
	buffer_info = &rx_ring->buffer_info[i];
	//printk("e1000_clean_rx_irq_ps staterr = %d" , staterr);
	while (staterr & E1000_RXD_STAT_DD) {
		if (*work_done >= work_to_do)
			break;
		(*work_done)++;
				     ...
				     int work_to_do)
{
	...
	staterr = le32_to_cpu(rx_desc->wb.upper.status_error);
	buffer_info = &rx_ring->buffer_info[i];
	//printk("e1000_clean_jumbo_rx_irq staterr = %d" , staterr);
	while (staterr & E1000_RXD_STAT_DD) {
		struct sk_buff *skb;
				    ...
				    struct net_device *netdev)
{
	...
	if (count) {
		...
		//writel(tx_ring->next_to_use, tx_ring->tail);
		//printk("writel(tx_ring->next_to_use, tx_ring->tail);");
	}
	//printk("e1000_xmit_frame NETDEV_TX_OK");
	return NETDEV_TX_OK;
}
...
MODULE_AUTHOR("EtherCAT-capable Intel Corporation, <linux.nics@intel.com>");
