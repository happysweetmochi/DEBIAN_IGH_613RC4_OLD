...
{
    //printk("ec_ioctl_activate uper step 1\n");
    ec_ioctl_master_activate_t io;
    ec_domain_t *domain;
    off_t offset;
    int ret;
    ...
    //printk("ec_ioctl_activate uper step 2\n");
    if (!ctx->process_data)
    {
        io.process_data = NULL;
        ...
        ctx->process_data_size = 0;
        //printk("ec_ioctl_activate uper step 3\n");
        if (ec_lock_down_interruptible(&master->domains_lock))
            return -EINTR;
        ...
        //printk("ec_ioctl_activate uper step 4\n");
        if (ctx->process_data_size) {
            ctx->process_data = vmalloc(ctx->process_data_size);
            if (!ctx->process_data) {
                ctx->process_data_size = 0;
        ...
        //printk("ec_ioctl_activate uper step 5\n");
        ec_lock_up(&master->domains_lock);
    ...
    //printk("ec_ioctl_activate uper step 6\n");
    ...
    if (ret < 0)
        return ret;
    //printk("ec_ioctl_activate uper step 7\n");
    if (copy_to_user((void __user *) arg, &io,
                sizeof(ec_ioctl_master_activate_t)))
        return -EFAULT;
    //printk("ec_ioctl_activate uper step 8\n");
    return 0;
}
        ...
        )
{
    ...
    switch (cmd) {
        ...
        case EC_IOCTL_ACTIVATE:
            ...
            printk("run in ec_ioctl_activate uper\n");
