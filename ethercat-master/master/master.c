...
/*
// 内核中的简单 strtok_r 实现
static char *strtok_r_kernel(char *str, const char *delim, char **saveptr) {
    char *token;
    
    printk(KERN_ERR "str =  %d\n", (unsigned long)str);
    printk(KERN_ERR "saveptr =  %d\n", (unsigned long)saveptr);
    printk(KERN_ERR "*saveptr =  %d\n", (unsigned long)*saveptr);
...
static int parse_key_value(const char *line, struct kv_pair *pair) {
    char *sep, *endptr;
    ...
    sep = strchr(line, '=');
    if (!sep) {
        printk(KERN_ERR "parse_key_value: Missing '=' in line: %s\n", line);
        return -EINVAL;
    }

    // 提取 key
    if (sep - line >= MAX_KEY_LEN) {
        printk(KERN_ERR "parse_key_value: Key too long in line: %s\n", line);
        return -EINVAL;
    }
    strncpy(pair->key, line, sep - line);
    pair->key[sep - line] = '\0';
    strim(pair->key); // 去掉前后空白字符

    if (*pair->key == '\0') {
        printk(KERN_ERR "parse_key_value: Empty key in line: %s\n", line);
        return -EINVAL;
    }

    // 提取 value 并转换为 int
    pair->value = simple_strtol(sep + 1, &endptr, 10);

    // 检查 value 是否有效
    if (*endptr != '\0' && *endptr != '\n') {
        printk(KERN_ERR "parse_key_value: Invalid value in line: %s\n", line);
        return -EINVAL;
    }

    // 额外检查 value 范围（如果有特定需求）
    if (pair->value < INT_MIN || pair->value > INT_MAX) {
        printk(KERN_ERR "parse_key_value: Value out of range in line: %s\n", line);
        return -ERANGE;
    }
...
int load_config_file(const char *filepath, struct kv_pair *kv_array) {
    ...
    for(int i = 0;i < MAX_KV_PAIRS;i++){
        kv_array[i].value = -1;
        memset(kv_array[i].key,0x0,MAX_KEY_LEN);
    }
    // 分配缓冲区
    //buffer = kmalloc(PAGE_SIZE, GFP_KERNEL);
    printk(KERN_INFO "kzalloc(PAGE_SIZE,GFP_ATOMIC)");
    ...
    // 打开文件
    printk(KERN_INFO "filp_open(filepath, O_RDONLY, 0)");
    ...
    // 读取文件内容
    printk(KERN_INFO "kernel_read(file, buffer, PAGE_SIZE - 1, &pos)");
    read_bytes = kernel_read(file, buffer, PAGE_SIZE - 1, &pos);
    if (read_bytes < 0) {
        filp_close(file, NULL);
        //kfree(buffer);
        return -4;
    }
    buffer[read_bytes] = '\0'; // 确保缓冲区是以 NULL 结尾的字符串

    // 按行解析
    printk(KERN_INFO "strtok_r_kernel(buffer, , &saveptr)");
    line = strtok_r_kernel(buffer, "\n", &saveptr);
    while (line && kv_count < MAX_KV_PAIRS) {
    //while (line && kv_count < 7) {
        // 安全检查
         
        if (!line) {
            printk(KERN_ERR "NULL line encountered\n");
            break;
        }
         printk(KERN_INFO "strim(line)");
        strim(line);
        if (*line == '\0') { // 跳过空行
            line = strtok_r_kernel(NULL, "\n", &saveptr);
            continue;
        }
        printk(KERN_INFO "parse_key_value(line, &kv_array[kv_count]) ");
        if (parse_key_value(line, &kv_array[kv_count]) == 0) {
            kv_count++;
        } else {
            printk(KERN_ERR "Failed to parse line: %s\n", line);
            break;
        }
        printk(KERN_INFO "strtok_r_kernel(NULL, , &saveptr)");
        line = strtok_r_kernel(NULL, "\n", &saveptr);

    }

    printk(KERN_INFO "filp_close(file, NULL)");
    // 关闭文件并释放内存
    filp_close(file, NULL);
    //kfree(buffer);
...
*/
int cpumaskinit = 0;
//struct kv_pair kv_array[MAX_KV_PAIRS];
...
int set_cpus_chang(void){
    ...
    int confret = 0;
    if(cpumaskinit == 0){
        confret = load_config_file(CONFIG_FILE, kv_array);
        if (confret < 0) {
            printk(KERN_ERR "Failed to load configuration: %d\n", confret);
            //return;
        }   
        cpumaskinit = 1;    
    }
    ...
    cpumask_clear(&mask);
    for(int i = 0;i < MAX_KV_PAIRS;i++){
        if(strcmp(kv_array[i].key, "ighcore") == 0){
            printk(KERN_ERR "ighcore : %d" , kv_array[i].value);
            cpumask_set_cpu(kv_array[i].value, &mask); 
        }
    }
